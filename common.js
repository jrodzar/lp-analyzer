...
// Para cada mes natural cubierto por una serie diaria de fees acumuladas +
// capital invertido, calcula el APR resultante anualizando linealmente:
//
//   APR_mes = (fees_del_mes / capital_medio_del_mes) × (365 / días_activos) × 100
//
// `points` es el shape estándar `timelineSeries`:
//   [{ ts (ms), feesUSD (acumulado), depositedUSD (acumulado), withdrawnUSD (acumulado) }]
//
// `fees_del_mes` se calcula como el delta del cumulative al cierre del mes
// vs el cierre del mes anterior. `capital_medio` es la media de
// max(0, depositedUSD - withdrawnUSD) sobre los puntos del mes. `días_activos`
// son los días que el snapshot estaba activo durante el mes (1 si solo hubo
// un punto). El mes en curso se marca con `isOngoing: true`.
//
// Devuelve: [{ monthKey, monthLabel, feesUSD, capitalAvg, daysActive,
//             daysInMonth, apr, isOngoing }] ORDENADO del más reciente al más
// antiguo (el primero es el mes en curso).
//
// Alternativas para implementar más adelante (manteniendo esta data):
//   Opción B — Chart.js bar chart con APR por mes (visual).
//   Opción C — Línea APR rolling 30d sobre el gráfico existente de fees
//              acumuladas en eje Y secundario.
function computeMonthlyAPRs(points) {
  ...
  const daysInMonth = (ts) => {
    const d = new Date(ts);
    return new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth() + 1, 0)).getUTCDate();
  };

  // Agrupar puntos por mes (YYYY-MM en UTC)
  ...
  // Calcular delta-fees mes a mes y APR
  const result = [];
  ...
  for (let i = 0; i < months.length; i++) {
    ...
    // Mes en curso: el último mes Y el último punto cercano al presente (<7d).
    const isOngoing = (i === months.length - 1) && (nowMs - m.lastTs < 7 * 86400000);

    const apr = (capitalAvg > 0 && daysActive >= 1)
      ? (feesMonth / capitalAvg) * (365 / daysActive) * 100
      : null;
    ...
    result.push({
      ...
      capitalAvg,
      daysActive,
      daysInMonth: daysInMonth(m.firstTs),
      apr,
      isOngoing,
    });
...
// Renderiza una tabla compacta de APR por mes — para cards de posición y
// para el Histórico agregado. Si `rows` está vacío, devuelve "".
//
// opts:
//   · limit (number)        — mostrar solo los N meses más recientes
